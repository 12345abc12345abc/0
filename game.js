...
class Tower{
  ...
  update(dt,ores){
    ...
    if(tp==='aoe'){
      ...
      if(this._aoeT>=iv){this._aoeT=0;const rng=this.getRange()*TS;const rng2=rng*rng;const cx0=this.cx,cy0=this.cy;let hit=false;for(const o of ores){const _x=o.x-cx0,_y=o.y-cy0;if(o.alive&&_x*_x+_y*_y<=rng2){o.takeDmg(this.getDmg(),'aoe');hit=true;}}if(hit){this._firingT=.28;this._eff(new GridFlashEff(cx0,cy0,rng,this.color));SFX.shoot('aoe');}}return;}
  ...
  // 코어슈터: 원형 디스크 + X 페탈 핀 베이스 + 블록헤드 단포
  _dCS(ctx,r,t,f){
    const col=this.color;
    // BASE: circular disc + 4 diamond petal fins at X positions (4-way symmetric)
    const bR=r*.80;
    // Main disc
    const dg=ctx.createRadialGradient(0,0,0,0,0,bR);
    dg.addColorStop(0,'#1d1d1d');dg.addColorStop(.6,'#151515');dg.addColorStop(1,'#0f0f0f');
    ctx.beginPath();ctx.arc(0,0,bR,0,Math.PI*2);ctx.fillStyle=dg;ctx.fill();
    ctx.strokeStyle='#252525';ctx.lineWidth=1.6;ctx.stroke();
    // 4 diamond petal fins at 45°/135°/225°/315°
    for(let i=0;i<4;i++){
      ctx.save();ctx.rotate(i*Math.PI/2+Math.PI/4);
      const fw=bR*.34,innerY=bR*.62,outerY=bR*1.50;
      ctx.fillStyle='#1c1c1c';ctx.strokeStyle='#2a2a2a';ctx.lineWidth=1.2;
      ctx.beginPath();
      ctx.moveTo(0,innerY*.45);
      ctx.lineTo(-fw*.52,innerY);ctx.lineTo(-fw*.26,(innerY+outerY)*.5);
      ctx.lineTo(0,outerY);
      ctx.lineTo(fw*.26,(innerY+outerY)*.5);ctx.lineTo(fw*.52,innerY);
      ctx.closePath();ctx.fill();ctx.stroke();
      ctx.shadowColor=col;ctx.shadowBlur=f?10:2;
      ctx.strokeStyle=col+(f?'44':'12');ctx.lineWidth=1.0;ctx.stroke();ctx.shadowBlur=0;
      ctx.strokeStyle=col+(f?'2a':'08');ctx.lineWidth=0.7;
      ctx.beginPath();ctx.moveTo(0,innerY+r*.05);ctx.lineTo(0,outerY-r*.08);ctx.stroke();
      ctx.fillStyle='#1e1e1e';ctx.strokeStyle=col+(f?'44':'14');ctx.lineWidth=0.8;
      ctx.beginPath();ctx.arc(0,(innerY+outerY)*.5,r*.026,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.restore();
    }
    // Detail rings
    ctx.strokeStyle='#222';ctx.lineWidth=1.2;ctx.beginPath();ctx.arc(0,0,bR*.68,0,Math.PI*2);ctx.stroke();
    ctx.strokeStyle='#1c1c1c';ctx.lineWidth=0.9;ctx.beginPath();ctx.arc(0,0,bR*.40,0,Math.PI*2);ctx.stroke();
    // 8 bolts
    for(let i=0;i<8;i++){
      const a=i*Math.PI/4,bx=Math.cos(a)*bR*.54,by=Math.sin(a)*bR*.54;
    ...
    ctx.strokeStyle=col+(f?'55':'18');ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(0,0,bR,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;
    ctx.save();ctx.rotate(t*1.8);ctx.strokeStyle=f?col+'70':col+'22';ctx.lineWidth=1.4;ctx.setLineDash([r*.18,r*.10]);
    ctx.beginPath();ctx.arc(0,0,bR*.50,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.restore();
    ...
    // TURRET: compact wide-body blockhead cannon
    ctx.save();ctx.rotate(this.angle+Math.PI/2);
    // Main body — wide flat 8-sided housing
    ctx.fillStyle='#1e1e1e';ctx.strokeStyle=f?col+'70':col+'2a';ctx.lineWidth=1.9;
    ctx.beginPath();
    ctx.moveTo(-r*.50,r*.12);ctx.lineTo(r*.50,r*.12);
    ctx.lineTo(r*.46,r*.01);ctx.lineTo(r*.38,-r*.24);
    ctx.lineTo(r*.22,-r*.38);ctx.lineTo(-r*.22,-r*.38);
    ctx.lineTo(-r*.38,-r*.24);ctx.lineTo(-r*.46,r*.01);
    ctx.closePath();ctx.fill();ctx.stroke();
    // Side armor chevrons
    for(const sx of[-1,1]){
      ctx.fillStyle='#1a1a1a';ctx.strokeStyle=col+(f?'4a':'1c');ctx.lineWidth=0.9;
      ctx.beginPath();
      ctx.moveTo(sx*r*.46,r*.01);ctx.lineTo(sx*r*.64,-r*.06);
      ctx.lineTo(sx*r*.60,-r*.20);ctx.lineTo(sx*r*.38,-r*.18);
      ctx.closePath();ctx.fill();ctx.stroke();
      ctx.strokeStyle=col+(f?'36':'10');ctx.lineWidth=0.6;
      for(let v=0;v<2;v++){const vy=-r*.09-v*r*.058;ctx.beginPath();ctx.moveTo(sx*r*.48,vy);ctx.lineTo(sx*r*.60,vy);ctx.stroke();}
    }
    // Body detail: horizontal panel line + 2 indicator nodes
    ctx.strokeStyle=col+(f?'22':'09');ctx.lineWidth=0.8;
    ctx.beginPath();ctx.moveTo(-r*.36,-r*.14);ctx.lineTo(r*.36,-r*.14);ctx.stroke();
    for(const sx of[-1,1]){
      ctx.fillStyle='#232323';ctx.strokeStyle=col+(f?'66':'22');ctx.lineWidth=0.9;
      ctx.beginPath();ctx.arc(sx*r*.18,-r*.24,r*.034,0,Math.PI*2);ctx.fill();ctx.stroke();
      if(f){ctx.fillStyle=col+'aa';ctx.shadowColor=col;ctx.shadowBlur=9;ctx.beginPath();ctx.arc(sx*r*.18,-r*.24,r*.018,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}
    }
    // Barrel mount
    ctx.fillStyle='#1a1a1a';ctx.strokeStyle=f?col+'50':col+'20';ctx.lineWidth=1.4;
    ctx.beginPath();ctx.rect(-r*.24,-r*.38,r*.48,r*.11);ctx.fill();ctx.stroke();
    // SHORT wide barrel
    ...
    ctx.beginPath();ctx.rect(-brlHW,-r*.38-brlL,brlHW*2,brlL);ctx.fill();ctx.stroke();ctx.shadowBlur=0;
    // 1 barrel ring
    ctx.fillStyle='#212121';ctx.strokeStyle=col+(f?'66':'28');ctx.lineWidth=1.3;
    ctx.beginPath();ctx.rect(-brlHW*1.55,-r*.38-brlL*.55,brlHW*3.1,r*.038);ctx.fill();ctx.stroke();
    // Wide muzzle aperture
    ctx.shadowColor=col;ctx.shadowBlur=f?30:7;
    ctx.strokeStyle=col+(f?'cc':'55');ctx.lineWidth=2.4;
    ctx.beginPath();ctx.arc(0,-r*.38-brlL,brlHW*1.35,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;
    ctx.fillStyle='#0e0e0e';ctx.beginPath();ctx.arc(0,-r*.38-brlL,brlHW*.70,0,Math.PI*2);ctx.fill();
    if(f){
      ctx.globalAlpha=.85;
      const mg=ctx.createRadialGradient(0,-r*.38-brlL,0,0,-r*.38-brlL,r*.28);
      mg.addColorStop(0,'#fff');mg.addColorStop(.25,col);mg.addColorStop(1,col+'00');
      ctx.fillStyle=mg;ctx.beginPath();ctx.arc(0,-r*.38-brlL,r*.28,0,Math.PI*2);ctx.fill();
      ctx.globalAlpha=1;
    }
    ctx.restore();
    // Center core
  ...
  _dTwinHub(ctx,r,t){
    const col=this.color,f=this._firingT>0;
    // circular armored base
    ctx.beginPath();ctx.arc(0,0,r*.88,0,Math.PI*2);
    ctx.fillStyle='#141414';ctx.fill();
    ctx.strokeStyle=f?col+'99':'#3a3a40';ctx.lineWidth=2;ctx.stroke();
    // 8-segment panel inlays
    for(let i=0;i<8;i++){
      const a=i*Math.PI/4,a2=a+Math.PI/4;
      ctx.fillStyle=f&&i%2===0?col+'06':'#181818';
      ctx.beginPath();ctx.moveTo(0,0);ctx.arc(0,0,r*.86,a,a2);ctx.closePath();ctx.fill();
    }
    // inner circle ring
    ctx.strokeStyle=f?col+'28':col+'10';ctx.lineWidth=1;ctx.beginPath();ctx.arc(0,0,r*.6,0,Math.PI*2);ctx.stroke();
    // 4 radial strut channels (NSEW)
    for(let i=0;i<4;i++){
      const a=i*Math.PI/2;
      ctx.strokeStyle=f?col+'55':col+'22';ctx.lineWidth=3;ctx.lineCap='round';
      ctx.beginPath();ctx.moveTo(Math.cos(a)*r*.2,Math.sin(a)*r*.2);ctx.lineTo(Math.cos(a)*r*.62,Math.sin(a)*r*.62);ctx.stroke();
      ctx.strokeStyle=col+(f?'28':'10');ctx.lineWidth=1.5;
      ctx.beginPath();ctx.moveTo(Math.cos(a)*r*.22,Math.sin(a)*r*.22);ctx.lineTo(Math.cos(a)*r*.6,Math.sin(a)*r*.6);ctx.stroke();
    }
    // outer rotating dashed orbit-track ring
    ctx.save();ctx.rotate(t*.75);
    ctx.strokeStyle=col+(f?'55':'22');ctx.lineWidth=1.3;ctx.setLineDash([r*.22,r*.09]);
    ctx.beginPath();ctx.arc(0,0,r*.8,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.restore();
    // twin left/right hub nodes (E/W)
    for(const sx of[-1,1]){
      const nx=sx*r*.62;
      ctx.fillStyle='#1e1e1e';ctx.strokeStyle=f?col:col+'66';ctx.lineWidth=1.8;
      ctx.shadowColor=col;ctx.shadowBlur=f?20:6;
      ctx.beginPath();ctx.arc(nx,0,r*.19,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.shadowBlur=0;
      const ng=ctx.createRadialGradient(nx,0,0,nx,0,r*.19);
      ng.addColorStop(0,f?'#fff':col+'dd');ng.addColorStop(.45,col+'88');ng.addColorStop(1,col+'00');
      ctx.fillStyle=ng;ctx.beginPath();ctx.arc(nx,0,r*.19,0,Math.PI*2);ctx.fill();
      // node outer collar
      ctx.strokeStyle=col+(f?'33':'14');ctx.lineWidth=.9;ctx.setLineDash([2,2]);
      ctx.beginPath();ctx.arc(nx,0,r*.29,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);
      // node tick marks
      for(let i=0;i<4;i++){
        const a=i*Math.PI/2;
        ctx.strokeStyle=col+(f?'44':'18');ctx.lineWidth=0.8;
        ctx.beginPath();ctx.moveTo(nx+Math.cos(a)*r*.2,Math.sin(a)*r*.2);ctx.lineTo(nx+Math.cos(a)*r*.27,Math.sin(a)*r*.27);ctx.stroke();
      }
    }
    // central counter-rotating hex coil
    ctx.save();ctx.rotate(-t*1.8);
    ctx.strokeStyle=f?col:col+'66';ctx.lineWidth=1.6;ctx.shadowColor=col;ctx.shadowBlur=f?10:2;
    ctx.beginPath();
    for(let i=0;i<6;i++){const a=i/6*Math.PI*2;i===0?ctx.moveTo(Math.cos(a)*r*.28,Math.sin(a)*r*.28):ctx.lineTo(Math.cos(a)*r*.28,Math.sin(a)*r*.28);}
    ctx.closePath();ctx.stroke();ctx.shadowBlur=0;ctx.restore();
    // inner CW rotating triangle
    ctx.save();ctx.rotate(t*2.4);
    ctx.strokeStyle=col+(f?'55':'1c');ctx.lineWidth=1;
    ctx.beginPath();
    for(let i=0;i<3;i++){const a=i*Math.PI*2/3-Math.PI/6;i===0?ctx.moveTo(Math.cos(a)*r*.18,Math.sin(a)*r*.18):ctx.lineTo(Math.cos(a)*r*.18,Math.sin(a)*r*.18);}
    ctx.closePath();ctx.stroke();ctx.restore();
    // core
    ctx.shadowColor=col;ctx.shadowBlur=f?26:10;
    const cg=ctx.createRadialGradient(0,0,0,0,0,r*.19);
    cg.addColorStop(0,'#fff');cg.addColorStop(.4,col);cg.addColorStop(.75,col+'55');cg.addColorStop(1,col+'00');
    ctx.fillStyle=cg;ctx.beginPath();ctx.arc(0,0,r*.19,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle=f?col+'bb':col+'44';ctx.lineWidth=1.3;ctx.beginPath();ctx.arc(0,0,r*.26,0,Math.PI*2);ctx.stroke();
    ctx.shadowBlur=0;
  }
  _drawTwinOrbit(ctx){
